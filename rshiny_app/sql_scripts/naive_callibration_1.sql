-- Insert production volume data (transactional data)
INSERT INTO TB_Production_Volume(PeriodID, FinishedGoodID, CapacityVolume, BudgetedVolume)
	...
	FROM TB_Planning_Period
		JOIN TB_Production_Volume
			ON TB_Planning_Period.PreviousPeriodID = TB_Production_Volume.PeriodID
	ON CONFLICT (PeriodID, FinishedGoodID) DO NOTHING;
	
-- Insert operating expense data (transactional data)
INSERT INTO TB_Operating_Expense(PeriodID, AccountID, BudgetedExpense, Variator)
	SELECT TB_Planning_Period.PeriodID, AccountID, ActualExpense, Variator
	FROM TB_Planning_Period
		JOIN TB_Operating_Expense
			ON TB_Planning_Period.PreviousPeriodID = TB_Operating_Expense.PeriodID
...
-- Insert activity level data (transactional data)
...
-- Insert cost pool data (transactional data)
...
-- Insert activity pool data (transactional data) 
